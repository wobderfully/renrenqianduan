<template>
  <el-dialog :visible.sync="visible" :title="$t('mail.config')" :close-on-click-modal="false" :close-on-press-escape="false">
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmitHandle()" label-width="120px">
      <el-form-item prop="ssl" :label="$t('mail.ssl')">
        <el-select v-model="dataForm.ssl" :placeholder="$t('mail.ssl')">
          <el-option :label="$t('mail.ssl1')" :value="0"></el-option>
          <el-option :label="$t('mail.ssl2')" :value="1"></el-option>
        </el-select>
      </el-form-item>
